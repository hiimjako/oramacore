...
async fn test_collection_values_commit_and_reload() {
    ...
    let writer_values_file = writer_data_dir
        ...
        .join("collection_values.json");
    assert!(
        writer_values_file.exists(),
        "Collection values file should exist in writer: {writer_values_file:?}"
    );
    ...
    assert!(
        ...
        "Collection values file should exist in reader: {reader_values_file:?}"
    );
