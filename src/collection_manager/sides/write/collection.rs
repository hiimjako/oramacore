...
use oramacore_lib::collection_values::{CollectionValueOperation, CollectionValuesWriter};
...
pub struct CollectionWriter {
    ...
    values_writer: OramaAsyncLock<CollectionValuesWriter>,
}
...
impl CollectionWriter {
    pub async fn empty(
        ...
        Ok(Self {
            ...
            values_writer: OramaAsyncLock::new("values_writer", CollectionValuesWriter::empty()),
        })
    }
    ...
    pub async fn try_load(data_dir: PathBuf, context: WriteSideContext) -> Result<Self> {
        ...
        Ok(Self {
            ...
            values_writer: OramaAsyncLock::new(
                "values_writer",
                CollectionValuesWriter::try_load(data_dir.clone())
                    .context("Cannot load collection values")?,
            ),
        })
    }
    ...
    pub async fn commit(&mut self, data_dir: PathBuf) -> Result<()> {
        ...
        values_lock
            ...
            .context("Cannot commit collection values")?;
        drop(values_lock);
    ...
    pub async fn set_value(&self, key: String, value: String) -> Result<(), WriteError> {
        ...
        lock.set(key.clone(), value.clone())
            .context("Cannot set collection value")?;
        drop(lock);
        ...
        self.context
            ...
            .send(WriteOperation::Collection(
                self.id,
                CollectionWriteOperation::CollectionValue(CollectionValueOperation::Set {
                    key,
                    value,
                }),
            ))
            .await
            .context("Cannot send collection value set operation")?;
    ...
    pub async fn delete_value(&self, key: &str) -> Result<bool, WriteError> {
        ...
        if removed {
            self.context
                ...
                .send(WriteOperation::Collection(
                    self.id,
                    CollectionWriteOperation::CollectionValue(CollectionValueOperation::Delete {
                        key: key.to_string(),
                    }),
                ))
                .await
                .context("Cannot send collection value delete operation")?;
        }
